...
setInterval(rsg.updateSwitchStates, 500)
...
// Download button (floppy disk icon)
...
for (const Dbutton of downloadButton) {
    Dbutton.addEventListener("click", () => {
        ...
        // Generate raw link client-side using the current state
        ...
        .catch(() => {
            // Fallback: download the command as plain text from the DOM
            const commandSelector = rsg.uiElements[rsg.commandType].command;
            const commandText = document.querySelector(commandSelector).innerText;
...
// Tooltips disabled — no need for Popper tooltips anymore
// (they were appearing over buttons and blocking clicks)
